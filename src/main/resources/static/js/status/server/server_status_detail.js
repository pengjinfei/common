...
function renderJobs() {
    ...
    $("#server-jobs-tbl").bootstrapTable({
        url: "/elasticjob/servers/" + ip + "/jobs",
        cache: false,
        search: true,
        showRefresh: true,
        showColumns: true
...
function bindDisableButton() {
    ...
    $(document).on("click", "button[operation='disable-server-job'][data-toggle!='modal']", function(event) {
        $.ajax({
            url: "/elasticjob/servers/" + $("#server-ip").text() + "/jobs/" + $(event.currentTarget).attr("job-name") + "/disable",
            type: "POST",
            success: function() {
...
function bindShutdownButton() {
    ...
    $(document).on("click", "button[operation='shutdown-server-job'][data-toggle!='modal']", function(event) {
        ...
        $(document).on("click", "#confirm-btn", function() {
            $.ajax({
                url: "/elasticjob/servers/" + serverIp + "/jobs/" + jobName + "/shutdown",
                type: "POST",
                success: function () {
...
function bindRemoveButton() {
    ...
    $(document).on("click", "button[operation='remove-server-job'][data-toggle!='modal']", function(event) {
        ...
        $(document).on("click", "#confirm-btn", function() {
            $.ajax({
                url: "/elasticjob/servers/" + serverIp + "/jobs/" + jobName,
                type: "DELETE",
                success: function () {
                    $("#confirm-dialog").modal("hide");
                    $(".modal-backdrop").remove();
